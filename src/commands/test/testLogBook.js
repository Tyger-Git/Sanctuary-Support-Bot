...
function createEmbedFromChunk(chunk, pageIndex, totalPages) {
    const embed = new EmbedBuilder()
        .setTitle(`Page ${pageIndex + 1} of ${totalPages}`);

    chunk.forEach(message => {
        const key = Object.keys(message)[1];
        const isMod = message.userOrMod === "mod" ? "⬅" : "➡";
        embed.addFields(
            { name: `${isMod} ${message.userOrMod}`, value: message[key] }
        );
    });
...
module.exports = {
    ...
    callback: async (client, interaction) => {
        ...
        /*
        // DB emulation array builder
        let messages = [];

        for (let i = 1; i <= 100; i++) {
        messages.push({
            "who": "userOrMod",
            [`message${i}`]: `This is message ${i}`
        });
        }
